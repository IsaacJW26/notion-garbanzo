...
async function setInitialGitHubToNotionIdMap() {
  ...
  for (const { pageId, due_date } of currentPages) {
    notionPageWithDueDate[pageId] = due_date;
    console.log(`pageId ${pageId}, is due ${due_date}`);
  }
}
...
async function getPageFromNotionDatabase() {
  ...
  return pages.map(page => {
    return {
      pageId: page.id,
      due_date: page.properties["DUE"].number,
    }
  })
}
